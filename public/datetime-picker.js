/**
 * Created by Lazar on 2017-11-27.
 */
...
angular.module('ui.bootstrap.datetimepicker', ['ui.bootstrap.dateparser', 'ui.bootstrap.position'])
    ...
    .controller('DateTimePickerController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$document', '$timeout', '$uibPosition', 'dateFilter', 'testDateParser', 'uiDatetimePickerConfig', '$rootScope',
        function ($scope, $element, $attrs, $compile, $parse, $document, $timeout, $uibPosition, dateFilter, testDateParser, uiDatetimePickerConfig, $rootScope) {
            ...
            this.init = function (_ngModel) {
                ...
                if (!isHtml5DateInput) {
                    ...
                    ngModel.$formatters.push(function (value) {
                        ...
                        if(typeof testDateParser.fromTimezone == 'function')
                            $scope.date = testDateParser.fromTimezone(value, ngModelOptions.timezone);

                        dateFormat = dateFormat.replace(/M!/, 'MM')
                            .replace(/d!/, 'dd');

                        return testDateParser.filter($scope.date, dateFormat);
                    });
                } else {
                    ngModel.$formatters.push(function (value) {
                        if(typeof testDateParser.fromTimezone == 'function')
                ...
                function saveAsParser(value) {
                    ...
                    if (!isHtml5DateInput) {
                        dateFormat = dateFormat.replace(/M!/, 'MM')
                            .replace(/d!/, 'dd');
                        if(typeof testDateParser.fromTimezone == 'function')
                            return testDateParser.filter(testDateParser.fromTimezone(value, ngModelOptions.timezone), dateFormat);
                        return true;
                    } else {
                        if(typeof testDateParser.fromTimezone == 'function')
                            return testDateParser.fromTimezone(value, ngModelOptions.timezone).toLocaleString();
                        return true;
                    }
                }
            };
            ...
            $scope.isDisabled = function (date)
            {
                if (date === 'today' || date === 'now')
                    if(typeof testDateParser.fromTimezone == 'function')
                        date = testDateParser.fromTimezone(new Date(), timezone);
                ...
                angular.forEach(['minDate', 'maxDate'], function (key) {
                    if (!$scope.datepickerOptions[key]) {
                        ...
                        if(typeof testDateParser.fromTimezone == 'function')
                            dates[key] = testDateParser.fromTimezone(new Date($scope.datepickerOptions[key]), timezone);
                    } else {
            ...
            function parseDateString(viewValue) {
                var date = testDateParser.parse(viewValue, dateFormat, $scope.date);
                if (isNaN(date)) {
                    for (var i = 0; i < altInputFormats.length; i++) {
                        date = testDateParser.parse(viewValue, altInputFormats[i], $scope.date);
                        if (!isNaN(date)) {
                            return date;
                        }
                    }
            ...
            function parseDate(viewValue) {
                ...
                if (angular.isString(viewValue)) {
                    ...
                    if (!isNaN(date)) {
                        if(typeof testDateParser.toTimezone == 'function')
                            return testDateParser.toTimezone(date, ngModelOptions.timezone);
                    }
